...
void upgma_find_nearest_clusters(const dist_matrix *dmat, uint32_t *c1, uint32_t *c2) {
     ...
     *      min { D(c1, c2)  } among all c1, c2 in dmat
     */
    double min_distance = INFINITY;

    for (uint32_t i = 0; i < dmat->species_count; i++) {
        for (uint32_t j = 0; j < i; j++) {
            double distance = dist_matrix_distance(dmat, i, j);
    ...
    for (uint32_t i = 0; i < dmat->species_count; i++) {
        ...
        for (uint32_t j = 0; j < i; j++) {
            ...
            if (i == c1) {
                ...
                // Here's the only difference against NJ
                distance = (d1j + d2j) / 2;
            } else if (j == c1) {
...
// This is exactly the same than NJ
//
btree_storage *upgma_tree_init(const dist_matrix *dmat, btree_node **leafs) {
    return nj_tree_init(dmat, leafs);
}

// In UPGMA, we remove the usage of the parameter u
//
void upgma_tree_add_node(const dist_matrix *dmat, btree_storage *storage, btree_node **partial_trees, const char *name, uint32_t c1, uint32_t c2) {
    ...
    node->distance_left = (distance) / 2;
    node->distance_right = (distance) / 2;
