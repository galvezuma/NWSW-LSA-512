...
//	if (x==1 && y==1) exit(0);
	...
	if (y < gd->jobTable.numFragments_Y - 1) {
		...
		jobBelow->ptrRow = (gd->pass == ONLY_SCORE)? jobDone->ptrRow : gd->jobTable.rows[y+1] + x * gd->jobTable.fragmentSize_X;
		memcpy(jobBelow->ptrRow, resultingFragmentX, sizeof(struct Node)*(1+jobBelow->realSize_X));
	...
	if (x < gd->jobTable.numFragments_X - 1) {
		...
		jobRight->ptrColumn = (gd->pass == ONLY_SCORE)? jobDone->ptrColumn : gd->jobTable.columns[x+1] + y * gd->jobTable.fragmentSize_Y;
		memcpy(jobRight->ptrColumn, resultingFragmentY, sizeof(struct Node)*(1+jobRight->realSize_Y));
